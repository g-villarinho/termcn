...
function getSavedPM(): PM {
	try {
		...
		if (v && (PACKAGE_MANAGERS as readonly string[]).includes(v))
			return v as PM;
	} catch {}
	return "npm";
}
...
function InstallTabs({ pkg, className }: InstallTabsProps) {
	...
	function selectPM(v: string) {
		...
		setActive(pm);
		try {
			localStorage.setItem(LS_KEY, pm);
		} catch {}
	}
		...
		>
			...
			<div className="flex items-stretch border-b border-border bg-background1">
				...
				>
					...
					<span className="flex w-3.25 h-3.25 items-center justify-center leading-none">
			...
			{PACKAGE_MANAGERS.map((pm) => (
				...
				>
					...
					<span className="text-foreground1">
						&nbsp;shadcn@latest add&nbsp;
					</span>
